  ...
  // Try SPI clock in a fast one as possible up to 40MHz (M8266WIFI could support only upto 40MHz SPI) 
	/* Notes: 
	     As per STM32, SPI1/4 Clock Divided from APB2 Clock, SPI1/4 CLOCK = APB2_CLOCK/SPI_BaudRatePrescaler
	         ...
	         For STM32F4, APB2_CLOCK = PCLK2 = HCLK/2 = SYSCLK/2, APB1_CLOCK=PCLK1 = HCLK/4 = SYSCLK/4! please see system_stm32f4xx.c, 
...
*/
		...
		M8266HostIf_SPI_SetSpeed(SPI_BaudRatePrescaler_64);				// Setup SPI Clock. Here 84/4 = 21.0MHz for STM32F4xx SPI1 or SPI4, up to 40MHz, since SPI1/4 clock devided from faster APB2 clock
  #elif (M8266WIFI_SPI_INTERFACE_NO == 2) || (M8266WIFI_SPI_INTERFACE_NO == 3)
		M8266HostIf_SPI_SetSpeed(SPI_BaudRatePrescaler_64);				// Setup SPI Clock. Here 42/2 = 21.0MHz for STM32F4xx SPI2 or SPI3, up to 21MHz, since SPI2/3 clock devided from lowver APB1 clock
	#endif
	 spi_clk = 84000000/64;    // =clk/scl
...
#if 1  // Used to evaluate the high-speed spi communication. Changed to #if 0 to comment it for formal release
{
	 ...
	 if(M8266WIFI_SPI_Interface_Communication_OK(&byte)==0) 	  									//	if SPI logical Communication failed
   {
	...
	if(M8266WIFI_SPI_Set_Tx_Max_Power(68, &status)==0)   // tx_max_power=68 to set the max tx power of around half of manufacture default, i.e. 50mW or 17dBm
	  return 0;
#endif	
